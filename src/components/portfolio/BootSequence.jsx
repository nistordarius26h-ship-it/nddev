...
export function BootSequence({ onDone }) {
  ...
  useEffect(() => {
    ...
    const finish = setTimeout(() => {
      ...
      arr.push(hide);
    }, BOOT_LINES.length * stepMs + (reduce ? 120 : 450));
    arr.push(finish);
    timers.current = arr;
      ...
      <div className="flex-1 overflow-hidden mono text-[11px] sm:text-xs leading-relaxed text-white/70">
        ...
        {!fading && (
          <span
            ...
            style={{ animation: "pulse-pip 1s ease-in-out infinite" }}
          />
        )}
      </div>
      <div className="mt-4">
        ...
        <p className="mt-3 mono text-[10px] text-white/30">click to skip</p>
      </div>
    </div>
  );
}